...
import { MemoryMatch, type MatchPair } from "@/components/games/memory-match";

export const metadata = { title: "Term Match" };

export default async function MatchPage() {
  const supabase = await createClient();
  const { data: terms } = await supabase
    .from("glossary_terms")
    .select("term, short_blurb");

  const pairs: MatchPair[] = (terms ?? []).map((t) => ({
    term: t.term,
    definition: t.short_blurb,
  }));
